...
_Extends Supabase `auth.users`. Handles user identity._
...
_The structural backbone of the learning content._
...
_The actual quiz content. Uses JSONB for flexible answer structures._
...
_Transactional data generated by students. NEVER DELETED - Union merge strategy._
...
_Tracks student practice sessions for analytics and resumption._
...
_Pre-computed progress per student per skill. Updated on each attempt. Critical for mastery tracking._
...
_Critical for offline-first architecture. These tables exist both server-side (for reference) and client-side (Drift/SQLite)._
...
_Singleton table for tracking curriculum version and publish state._
...
_Critical for the synchronization engine._
...
_Every table must use this trigger to ensure `updated_at` is reliable._
...
_Critical for anonymous auth: When a new user signs up (including via `signInAnonymously()`), this trigger creates their profile row automatically._
...
_This is the PRIMARY and ONLY way students submit attempts. Do NOT use REST `/attempts` for student submissions._
...
_Groups and Assignments for Classroom Management._
...
_Tables for identity recovery and audit._
