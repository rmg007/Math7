<#
.SYNOPSIS
    Questerix Student App Deployment Orchestrator
.DESCRIPTION
    Single-command deployment pipeline for the Student App:
    - Student App (Flutter Web)
    - Supabase (Schema migrations + Edge Functions)
.PARAMETER Env
    Environment to deploy to: 'production' or 'test'
.PARAMETER Target
    Which target to deploy: 'questerix-student-app' (default)
...
.EXAMPLE
    ./orchestrator.ps1 -Env test
    ./orchestrator.ps1 -Env production -ConfirmProd
...
# Standalone: only the Student App is deployed from this repository
$SkipLanding = $true
...
function Invoke-PhaseBuild {
    ...
    $studentBuild = Join-Path $ScriptDir 'build/web'
    
    if (Test-Path $studentBuild) { Remove-Item -Recurse -Force $studentBuild }
    
    # Build student app at root
...
function Invoke-PhaseCleanup {
    ...
    Write-Host ""
    Write-Info "Live URL:"
    Write-Info "  Student App: https://$cfStudent.pages.dev"
    Write-Host ""
...
function Main {
    ...
    if ($Env -eq 'production' -and -not $ConfirmProd) {
        ...
        Write-Err "Example: ./orchestrator.ps1 -Env production -ConfirmProd"
        exit 1
    }
