...
6.  **Self-Review**: After each chunk, run static analysis and "hallucination check."
7.  **Fix Loop**: If errors, inconsistencies, or "spaghetti smells" are found, fix them immediately.
    - **Micro-Postmortem**: If you fixed a bug, **IMMEDIATELY** add a bullet point to `docs/LEARNING_LOG.md` with the Root Cause, Fix, and **Learning Flag** (`[need test]`, `[test created]`, or `[no test needed]`).
8.  **Preventative Measures (Crucial)**:
    - **If you fixed a bug, you MUST add a preventative test.**
    - **Rule**: "The bug should have failed a test. Write that test now."
    - **Action**: Create a specific test case that reproduces the bug, then verify it passes with your fix.
9.  **Repeat**: Loop until all planned features are implemented and the Agent is 100% confident.
...
11. **Visual Design Audit (If UI changed)**:
...
12. **Repeat**: Loop until **100% of tests pass**, **Visuals are Premium**, and **Performance is Smooth**.
13. **Evidence**: Provide a summary of passed tests (pass/fail counts) and **attach visual proof** (screenshots).
...
1.  **Postmortem**: Complete the Bug Learning Loop for any issues caught during development.
    - Update `docs/LEARNING_LOG.md` with flags: `[need test]`, `[test created]`, or `[no test needed]`.
