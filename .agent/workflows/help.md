...
| `/autopilot` | Full autonomous mode (turbo permissions only)                         |
...
### `/loki` - Autonomous Developer (RARV Protocol)

**The "Founding Engineer" mode.** Agent plans, codes, tests, and iterates autonomously using the RARV cycle (Reason → Act → Reflect → Verify). Includes self-healing, circuit breakers ($10 budget, 25 iteration cap), and persistent state. Follows `/process` phases but removes human gates (except deployment). Config: `.antigravity/skills/loki-mode/`.

### `/autopilot` - Full Autonomous Execution

Enables all commands to auto-run (if IDE configured). Loki Mode uses autopilot permissions internally.
