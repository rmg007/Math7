...
export class RiskScorer {
  ...
  calculateScore(
    ...
    const dimensions = {
      ...
      coverageTrajectory: this.scoreCoverageTrajectory(
        previousGapCount,
        currentGapCount,
      ),
    };
  ...
  private scoreSmokeGate(results: Record<string, TaskResult>) {
    ...
    const smokeResults = smokeNames
      .map((name) =>
        Object.values(results).find((r) => r && r.name.toLowerCase() === name),
      )
  ...
  private scoreTypeSafety(results: Record<string, TaskResult>) {
    const tscResult = Object.values(results).find(
      (r) => r.name.toLowerCase() === "typescript strict",
    );
   ...
   */
  private scoreCoverageTrajectory(
    previousGapCount?: number,
    currentGapCount?: number,
  ) {
