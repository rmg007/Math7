        ...
        if (kind === SyntaxKind.FunctionDeclaration) {
          ...
          if (init && init.getKind() === SyntaxKind.ArrowFunction) {
            ...
            results.push({
              ...
              kind: "const",
              signature:
                typeText.length < 120 ? typeText : typeText.slice(0, 120) + "…",
...
export class SkeletonGenerator {
    ...
    for (const [group, groupFiles] of Object.entries(groups).sort()) {
      ...
      for (const file of groupFiles) {
        ...
        for (const exp of file.exports) {
          ...
          lines.push(
            `- **${exp.name}** \`${exp.kind}\` \`${exp.signature}\`${doc}`,
          );
        }
        lines.push("");
      }
  ...
  writeMarkdownSummary(report: SkeletonReport, outputPath: string) {
    ...
    for (const dir of sortedDirs) {
      const filesInDir = groups[dir].sort((a, b) =>
      ...
      for (let i = 0; i < filesInDir.length; i++) {
        ...
        const topExports = file.exports
          ...
          .join(", ");
        const more =
          file.exports.length > 5 ? ` (+${file.exports.length - 5} more)` : "";

        lines.push(
          `| ${i === 0 ? `**${dir}**` : ""} | \`${file.file}\` | ${topExports}${more} |`,
        );
      }
    }
  ...
  writeUtilityRegistry(report: SkeletonReport, outputPath: string): void {
    const utilityFiles = report.files.filter(
      (f) =>
        f.file.startsWith("hooks/") ||
    ...
    for (const file of utilityFiles) {
      for (const exp of file.exports) {
        ...
        lines.push(
          `| **${exp.name}** | \`${exp.kind}\` | \`${file.file}\` | \`${cleanSig}\` | ${cleanDoc} |`,
        );
      }
    }
