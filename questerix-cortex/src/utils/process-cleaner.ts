  ...
  private static killOrphanedAnalyzers() {
    try {
      if (process.platform === "win32") {
        ...
        execSync(
          `powershell -NoProfile -Command "Get-Process dart, flutter -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue"`,
        );
        ...
        execSync(
          `pgrep -f "questerix-cortex|dart|flutter" | grep -v ${process.pid} | xargs kill -9 2>/dev/null || true`,
        );
      }
    } catch {
      /* skip */
