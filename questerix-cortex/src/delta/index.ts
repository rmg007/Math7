...
export interface SurfaceMap {
  hooks: { name: string; file: string; hasTest: boolean; functions: string[] }[];
...
export class DeltaEngine {
  ...
  constructor(projectRoot: string) {
    ...
    this.previousSurfaceMapPath = path.join(projectRoot, 'questerix-cortex', 'outputs', '.prev_surface_map.json');
  }
  ...
  computeDelta(currentSurfaceMap: SurfaceMap, gitActivity?: any): DeltaResult {
    ...
    if (fs.existsSync(this.previousSurfaceMapPath)) {
      try {
        previousSurfaceMap = JSON.parse(fs.readFileSync(this.previousSurfaceMapPath, 'utf-8'));
      } catch (error) {
    ...
    const currentSurfaceMapPath = path.join(this.projectRoot, 'questerix-cortex', 'outputs', 'SURFACE_MAP.json');
    fs.writeFileSync(this.previousSurfaceMapPath, fs.readFileSync(currentSurfaceMapPath));
    ...
    const newGaps = currentSurfaceMap.gaps.filter(gap => !previousGaps.has(gap));
    const resolvedGaps = previousSurfaceMap.gaps.filter(gap => !currentGaps.has(gap));
  ...
  private getHotFiles(surfaceMap: SurfaceMap): string[] {
    try {
      ...
      const gitOutput = execSync('git log --since="24 hours" --name-only --pretty=format:""', {
  ...
  private getGitActivity() {
    try {
      ...
      const lastCommit = execSync('git log -1 --pretty=format:"%H|%s|%an|%ar"', {
        cwd: this.projectRoot,
        encoding: 'utf-8'
      }).trim();
