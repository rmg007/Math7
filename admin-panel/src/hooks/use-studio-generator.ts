...
function buildStudioPrompt(config: StudioConfig): string {
  const { domain, topic, count, difficultyMix, questionTypes, customInstructions } = config;
...
TASK: Generate exactly ${count} high-quality quiz questions focused exclusively on "${topic}".
...
QUALITY RULES:
- Every question must be about "${topic}" specifically — no tangential content.
...
export function useStudioGenerator() {
  ...
  const generateBatch = useCallback(async (config: StudioConfig): Promise<boolean> => {
    if (config.topic.trim().length < 3) {
      toast({ title: 'Topic too short', description: 'Please enter at least 3 characters.', variant: 'destructive' });
      return false;
    }
    if (config.questionTypes.length === 0) {
      toast({ title: 'No question types selected', description: 'Select at least one type.', variant: 'destructive' });
      return false;
    }
  ...
  const regenerateSingle = useCallback(async (index: number, config: StudioConfig): Promise<void> => {
  ...
  const keepAll = useCallback(() => setStagedQuestions((q) => q.map((c) => ({ ...c, kept: true }))), []);
  const removeAll = useCallback(() => setStagedQuestions((q) => q.map((c) => ({ ...c, kept: false }))), []);
