...
const navigationGroups: NavGroup[] = [
  {
    title: 'Overview',
    items: [{ name: 'Dashboard', href: '/dashboard', icon: BarChart3, superAdminOnly: true }],
  },
  {
    title: 'Curriculum',
    items: [
  ...
  return (
    <div
      className={cn(
        ...
        isMobile &&
          'fixed inset-y-0 left-0 z-50 transform transition-transform duration-300 ease-in-out shadow-2xl',
    ...
    >
      ...
      >
        ...
        >
          <button
            ...
            aria-label={isSidebarCollapsed ? 'Expand sidebar' : 'Questerix Logo'}
            className={cn(
          ...
          {!isSidebarCollapsed && (
            <h1 className="text-lg font-bold text-white tracking-tight leading-none animate-in fade-in duration-300">
              Questerix
            </h1>
          )}
        </div>
      ...
      <nav className="flex-1 px-3 py-4 space-y-4 overflow-y-auto sidebar">
        {navigationGroups.map((group) => {
          ...
          const matchGroupPath = visibleItems.some(
            (item) =>
                  ...
                  {visibleItems.map((item) => {
                        ...
                        >
                          <item.icon
                            className={cn(
                              ...
                              isActive
                                ? 'text-white'
                                : 'text-purple-400/70 group-hover:text-purple-200'
                            )}
                          />
                          {!isSidebarCollapsed && (
                    ...
                    return (
                      ...
                      >
                        ...
                        />
                        {!isSidebarCollapsed && <span className="flex-1 text-sm">{item.name}</span>}
                        {isActive && !isSidebarCollapsed && (
                          <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 bg-gradient-to-b from-purple-400 to-blue-400 rounded-r-full shadow-[0_0_10px_rgba(192,132,252,0.6)]" />
                        )}
                      </Link>
