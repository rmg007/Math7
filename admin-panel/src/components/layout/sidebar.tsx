...
export function Sidebar({ isOpen = true, onClose, isMobile = false }: SidebarProps) {
  ...
  const { currentApp, setCurrentApp, apps, isLoading: appsLoading, isSidebarCollapsed: appContextCollapsed, toggleSidebar } = useApp()
  ...
  // Sync with AppContext if necessary, but here we use local state or context
  // Preference: use the context's isSidebarCollapsed if it exists.
  const effectiveCollapsed = !isMobile && (isCollapsed || appContextCollapsed)
  ...
  const toggleGroupToggle = useCallback((groupId: string) => {
    setGroupState((prev) => {
  ...
  return (
    ...
    >
      ...
      {/* ============ App Selector ============ */}
      {effectiveCollapsed ? (
        <div className="py-4 flex justify-center border-b border-white/5 bg-white/[0.03]">
          <div 
      ...
      <nav className="flex-1 py-1.5 overflow-y-auto custom-scrollbar">
        {visibleGroups.map((group) => (
          <NavSection
            ...
            onToggle={() => toggleGroupToggle(group.id)}
      ...
      <div className="px-3 py-2.5 border-t border-white/10 bg-black/20 backdrop-blur-md">
        ...
        {!userInfo && (
          <button
            ...
            className={cn(
               'flex items-center gap-3 rounded-xl text-sm font-medium text-purple-200 hover:bg-white/5 hover:text-white transition-all',
               effectiveCollapsed ? 'p-2 justify-center' : 'px-4 py-2.5 w-full'
            )}
          >
