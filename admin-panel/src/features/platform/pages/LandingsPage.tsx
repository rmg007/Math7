...
import { Globe, Pencil, Save, ChevronLeft, Plus, LayoutPanelTop, Search, X, Monitor, Megaphone, Terminal, Activity, ArrowRight } from 'lucide-react';
import { useLandingPages, useUpdateLandingPage, useCreateLandingPage, type LandingPageWithApp } from '../hooks/use-landings';
...
export function LandingsPage() {
  ...
  const handleCreate = async () => {
    if (!selectedAppId) {
      toast({ title: "Error", description: "Please select a target application", variant: "destructive" });
      return;
    }
    ...
    if (existing) {
      toast({ title: "Notice", description: "This app already has a landing registry. Redirecting to editor." });
      handleEdit(existing);
    ...
    try {
      ...
      toast({ title: "Error", description: "Failed to initialize landing registry", variant: "destructive" });
    }
  };
      ...
      <div className="max-w-7xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 p-4 md:p-8">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-6">
            <Button variant="ghost" size="icon" onClick={() => setEditingLanding(null)} className="h-12 w-12 rounded-2xl bg-white/50 border border-white/20 hover:bg-white transition-all">
              <ChevronLeft className="w-6 h-6 text-gray-600" />
            </Button>
            <div>
              <h2 className="text-3xl font-black text-gray-900 tracking-tight italic">Content Editor</h2>
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mt-1">Refining: {editingLanding.apps?.display_name}</p>
            </div>
          </div>
          
          <div className="flex items-center gap-3">
            <Button variant="ghost" onClick={() => setEditingLanding(null)} className="h-12 px-8 rounded-2xl font-black text-[10px] uppercase tracking-widest text-gray-400 hover:bg-gray-100 italic transition-all">
              Abort Editing
            </Button>
            <Button onClick={handleSave} className="h-12 px-10 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-black text-[10px] uppercase tracking-widest shadow-lg shadow-indigo-600/20 transition-all hover:-translate-y-0.5" disabled={updateLanding.isPending}>
          ...
          <div className="bg-white/70 backdrop-blur-xl rounded-[2.5rem] p-10 shadow-sm border border-white/20 space-y-8">
            <div className="flex items-center gap-4 mb-2">
              ...
              <div>
                ...
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest italic">User-facing brand messaging</p>
              </div>
            </div>

            <div className="space-y-6">
              <div className="space-y-2 group">
                <Label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Primary Headline</Label>
                <Input 
              ...
              <div className="space-y-2 group">
                <Label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Secondary Narrative</Label>
                <Textarea 
                  rows={6}
          ...
          <div className="bg-white/70 backdrop-blur-xl rounded-[2.5rem] p-10 shadow-sm border border-white/20 space-y-8">
             <div className="flex items-center gap-4 mb-2">
              ...
              <div>
                ...
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest italic">Search engine index parameters</p>
              </div>
            </div>

            <div className="space-y-6">
              <div className="space-y-2 group">
                <Label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Meta Title Tag</Label>
                <Input 
              ...
              <div className="space-y-2 group">
                <Label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Crawler Description</Label>
                <Textarea 
                  rows={6}
  ...
  const paginatedLandings = filteredLandings.slice((currentPage - 1) * pageSize, currentPage * pageSize);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 p-4 md:p-8">
      <AdminHeader 
        ...
        icon={Globe}
        actions={
          <Button onClick={() => setIsCreateDialogOpen(true)} className="h-12 px-8 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-black text-[10px] uppercase tracking-widest shadow-lg shadow-indigo-600/20 transition-all hover:-translate-y-0.5 gap-3">
      ...
      <div className="bg-white/70 backdrop-blur-xl rounded-[2rem] shadow-sm border border-white/20 p-6 flex flex-col md:flex-row gap-6 items-center">
        ...
        <div className="flex items-center gap-3 shrink-0">
          <div className="px-4 py-2 bg-indigo-500/10 border border-indigo-500/10 rounded-xl flex items-center gap-2">
             <span className="text-[10px] font-black text-indigo-500 uppercase tracking-widest">Registries:</span>
             <span className="text-sm font-black text-indigo-700 tracking-tight">{filteredLandings.length} ACTIVE</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white/70 backdrop-blur-xl rounded-[2.5rem] shadow-sm border border-white/20 overflow-hidden hover:shadow-xl transition-all duration-500">
        <div className="px-8 py-6 border-b border-gray-100 bg-gray-50/30 flex items-center justify-between">
          <div>
            <h3 className="text-xl font-black text-gray-900 tracking-tight italic">Content Distribution Network</h3>
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mt-1 italic">Multi-Tenant Marketing Nodes</p>
          </div>
          ...
          <Table className="w-full">
            <TableHeader>
              <TableRow className="bg-gray-50/50 hover:bg-gray-50/50 border-b-2 border-gray-100">
                <TableHead className="font-black text-[10px] uppercase tracking-widest text-gray-400 px-8 h-14">Target Application</TableHead>
                <TableHead className="font-black text-[10px] uppercase tracking-widest text-gray-400 h-14">Deployment URL</TableHead>
                <TableHead className="font-black text-[10px] uppercase tracking-widest text-gray-400 h-14">Primary Hook</TableHead>
                <TableHead className="font-black text-[10px] uppercase tracking-widest text-gray-400 h-14">Node Status</TableHead>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow key={l.landing_page_id} className="group hover:bg-indigo-50/30 transition-colors border-b border-gray-50 last:border-0">
                  <TableCell className="px-8 py-5">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-black text-gray-900 tracking-tight text-base italic leading-none">{l.apps?.display_name}</p>
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1">Tenant ID: {l.app_id?.split('-')[0] || 'UNMAPPED'}</p>
                      </div>
                    </div>
                  </TableCell>
        ...
        <DialogContent className="rounded-[2.5rem] border-none bg-white/90 backdrop-blur-2xl p-0 overflow-hidden shadow-2xl">
          <div className="p-10 space-y-8">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-2xl font-black text-gray-900 tracking-tight italic">Initialize Registry</h2>
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1">Select application endpoint</p>
              </div>
            </div>

            <div className="space-y-4">
              <div className="space-y-2">
                <Label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-1">Target Tenant</Label>
                <Select value={selectedAppId} onValueChange={setSelectedAppId}>
                  ...
                  {apps?.filter(app => !landings?.some(l => l.app_id === app.app_id)).map(app => (
                    <SelectItem key={app.app_id} value={app.app_id} className="rounded-xl py-3 font-bold">
                        {app.display_name} ({app.subdomain})
                      </SelectItem>
                    ))}
                    {apps?.filter(app => !landings?.some(l => l.app_id === app.app_id)).length === 0 && (
                       <div className="p-4 text-xs font-black text-center text-rose-400 uppercase tracking-widest italic">
          ...
          <DialogFooter className="bg-gray-50/50 p-8 flex gap-3 border-t border-gray-100">
            <Button variant="ghost" onClick={() => setIsCreateDialogOpen(false)} className="h-12 px-8 rounded-2xl font-black text-[10px] uppercase tracking-widest text-gray-400 hover:bg-gray-200 italic transition-all">
              Abort
            </Button>
            <Button onClick={handleCreate} className="h-12 px-10 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-black text-[10px] uppercase tracking-widest shadow-lg shadow-indigo-600/20 transition-all hover:-translate-y-0.5" disabled={createLanding.isPending}>
