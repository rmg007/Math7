...
const UserRow = memo(({ user, currentUserId, onSelect, onDeactivate, onReactivate, isSelected }: UserRowProps) => {
  return (
    <TableRow key={user.id} className={cn("group hover:bg-indigo-50/30 transition-colors border-b border-gray-50/50 last:border-0", user.deleted_at && "opacity-40 grayscale-[0.5]")}>
      <TableCell className="pl-8 pr-2 py-5">
        <button 
          ...
          className={cn(
            "transition-colors",
            user.id === currentUserId ? "text-gray-100 cursor-not-allowed" : "text-gray-300 hover:text-indigo-600"
          )}
        >
          {isSelected ? <UserCheck className="h-5 w-5 text-indigo-600" /> : <Activity className="h-5 w-5 opacity-20" />}
        </button>
      </TableCell>
          ...
          <div>
            <div className="flex items-center gap-2">
               ...
               {user.id === currentUserId && (
                 <div className="px-1.5 py-0.5 rounded-md bg-indigo-600 text-white text-[8px] font-black uppercase tracking-widest">SELF</div>
               )}
            </div>
        ...
        <div className="flex items-center gap-2">
          {user.role === 'super_admin' ? (
            <div className="flex items-center gap-1.5 px-3 py-1 bg-purple-500/10 border border-purple-500/20 rounded-lg group/badge transition-all hover:bg-purple-500/20">
              ...
              <span className="text-[10px] font-black text-purple-700 uppercase tracking-widest">Level 10 Super</span>
            </div>
          ) : (
            <div className="flex items-center gap-1.5 px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded-lg group/badge transition-all hover:bg-emerald-500/20">
              ...
              <span className="text-[10px] font-black text-emerald-700 uppercase tracking-widest">Level 1 Admin</span>
            </div>
          )}
        </div>
      </TableCell>
      <TableCell className="px-6 py-5">
         <div className="flex items-center gap-2 text-gray-400">
            ...
            <span className="text-xs font-black uppercase tracking-wider">{formatDate(user.created_at)}</span>
         </div>
      </TableCell>
      <TableCell className="px-6 py-5">
         <StatusBadge 
...
export function UserManagementPage() {
  ...
  const handleReactivate = useCallback(async (userId: string) => {
    try {
      ...
      toast({ title: "Error", description: "Failed to restore operator access", variant: "destructive" });
    }
  }, [toast, fetchUsers]);

  const handleBulkToggleStatus = useCallback(async (status: 'deactivate' | 'reactivate') => {
    ...
    try {
      ...
      toast({ title: "Batch Error", description: "Operation failed for one or more operators.", variant: "destructive" });
    }
        ...
        actions={
          <div className="flex items-center gap-3 px-6 py-3 bg-indigo-500/10 border border-indigo-500/10 rounded-2xl">
            ...
            <div className="flex flex-col">
              <span className="text-sm font-black text-indigo-700 tracking-tight">{activeUsersCount} ACTIVE</span>
              <span className="text-[9px] font-black text-indigo-400 uppercase tracking-widest">Operators</span>
            </div>
          </div>
        }
      />
      ...
      <div className="bg-white/70 backdrop-blur-xl rounded-[2rem] shadow-sm border border-white/20 p-6 flex flex-col md:flex-row gap-6 items-center">
        ...
        <div className="flex items-center gap-3 shrink-0">
          <div className="px-4 py-2 bg-indigo-500/10 border border-indigo-500/10 rounded-xl flex items-center gap-2">
             ...
             <span className="text-sm font-black text-indigo-700 tracking-tight">{filteredUsers.length} RESULTS</span>
          </div>
        </div>
      </div>
      ...
      {selectedIds.size > 0 && (
        <div className="flex items-center justify-between p-4 bg-indigo-600 rounded-[2rem] shadow-xl shadow-indigo-600/20 animate-in slide-in-from-top-4 duration-500">
          <div className="flex items-center gap-4 pl-4">
            <span className="text-white font-black text-xs uppercase tracking-[0.2em]">{selectedIds.size} SELECTED FOR BATCH MODIFICATION</span>
          </div>
      ...
      <div className="bg-white/70 backdrop-blur-xl rounded-[2.5rem] shadow-sm border border-white/20 overflow-hidden hover:shadow-xl transition-all duration-500">
        <div className="px-8 py-6 border-b border-gray-100 bg-gray-50/30 flex items-center justify-between">
          <div>
            ...
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mt-1 italic">Verified System Personnel</p>
          </div>
              ...
              <TableRow className="bg-gray-50/50 hover:bg-gray-50/50 border-b-2 border-gray-100/50">
                <TableHead className="w-12 h-14 pl-8 pr-2">
                  <button onClick={handleSelectAll} className="text-gray-300 hover:text-indigo-600 transition-colors">
                    {selectedIds.size > 0 && selectedIds.size === paginatedUsers.filter(u => u.id !== currentUserId).length ? <History className="h-5 w-5 text-indigo-600" /> : <Activity className="h-5 w-5" />}
                  </button>
                </TableHead>
                <TableHead className="text-left py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest h-14">Operator Identity</TableHead>
                <TableHead className="text-left px-6 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest h-14">Access Tier</TableHead>
                <TableHead className="text-left px-6 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest h-14">Enlistment</TableHead>
                <TableHead className="text-left px-6 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest h-14">Duty Status</TableHead>
                <TableHead className="text-right px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest h-14">Commands</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
               {loading ? (
                  ...
                  <TableCell colSpan={6} className="py-20 p-0">
                    <EmptyState
                      ...
                      description={searchQuery 
                        ...
                        : "The operator directory is currently empty."}
                    />
                  </TableCell>
                </TableRow>
              ) : (
