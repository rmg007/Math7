...
const GroupCard = memo(({ group, onCopy, copiedId }: GroupCardProps) => {
  return (
    <div
      key={group.id}
      className="relative group overflow-hidden rounded-[2.5rem] border border-white/40 bg-white/70 backdrop-blur-xl hover:border-indigo-300 hover:shadow-2xl hover:shadow-indigo-500/10 transition-all duration-500 hover:-translate-y-2"
    >
      ...
      <div className="p-8 relative">
        ...
        <h3
          className="text-xl font-black text-gray-900 mb-2 line-clamp-1 tracking-tight"
          title={group.name}
        >
          {group.name}
        </h3>
      ...
      <div className="px-8 py-5 bg-gray-50/50 border-t border-gray-100 flex items-center justify-between group-hover:bg-indigo-500 transition-all duration-500">
        ...
        <Link
          to={`/groups/${group.id}`}
          className="text-xs font-black uppercase tracking-widest text-indigo-600 hover:text-indigo-700 p-2 group-hover:text-white group-hover:bg-white/10 rounded-xl transition-all"
        >
          Configure &rarr;
        </Link>
      </div>
      ...
      <AdminHeader
        ...
        actions={
          <Button
            asChild
            size="lg"
            className="bg-gradient-to-br from-indigo-600 to-purple-700 hover:from-indigo-700 hover:to-purple-800 text-white rounded-2xl px-6 py-6 shadow-lg shadow-indigo-600/20 font-black text-sm uppercase tracking-widest gap-2 transition-all hover:-translate-y-0.5 border-0"
          >
            <Link to="/groups/new">
