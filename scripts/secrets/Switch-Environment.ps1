...
try {
    ...
    Write-Host "  Proceeding despite secrets verification failure (Force)" -ForegroundColor Yellow
}

# Phase 2: Generate environment files
Write-Host " Phase 2: Generating environment files..." -ForegroundColor Yellow
...
if (-not (Test-Path (Join-Path $RootDir $configFile))) {
     if ($TargetEnvironment -eq "staging") {
         Write-Host "  master-config.staging.json not found, falling back to master-config.json" -ForegroundColor Yellow
