...
-- 2. HARDEN CONSUME_TENANT_TOKENS (USE UPDATED ADMIN CHECK)
-- ----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.consume_tenant_tokens(p_app_id uuid, p_tokens_used integer, p_operation text DEFAULT 'generate_questions'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
    v_quota RECORD;
BEGIN
    -- 1. Check if user is admin or it's a service call
    -- Note: jwt_is_tenant_admin() checks app_id, so we verify p_app_id matches
    IF NOT public.jwt_is_tenant_admin() 
       AND (auth.jwt() ->> 'role' != 'service_role') 
       AND NOT public.jwt_is_super_admin() THEN
        RETURN jsonb_build_object('success', false, 'message', 'Unauthorized');
    END IF;

    -- Ensure the admin is acting on their own app
    ...
    SET 
        current_token_usage = current_token_usage + p_tokens_used,
        updated_at = NOW()
    WHERE app_id = p_app_id;

    -- 6. Log usage for detailed tracking
    INSERT INTO public.ai_token_usage (app_id, user_id, operation, tokens_used)
    VALUES (p_app_id, auth.uid(), p_operation, p_tokens_used);
